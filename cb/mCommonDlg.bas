...
Public Function ShowOpenDialog(filestruct As stcFileStruct) As Boolean
    With filestruct
        ...
        If .strInitDir = Empty Then .strInitDir = GetWorkScanfileFolder
        ShowOpenDialog = VBGetOpenFileName(.strFileName, .strFileTitle, True, , .blnReadOnly, , .strFilter, .lngFilterIndex, .strInitDir, .strDialogtitle)
...
Private Sub cmdOpen_Click()
    ...
    File.strFilter = "Text Files *.txt|*.stl" '// use same format as
...
Public Function EspritProgramFolder() As String
    ' the contents of this folder are saved with the .esp file
    EspritProgramFolder = Application.Path
End Function
